//Create.js - TF2 Tags Item Creation Handler - 2015-08-17
...
$(document).ready(function (){
    $(".class-token").click(function() {
        classSelect($(this).attr('classname'));
    });
    
    $(".slot-token").click(function() {
    ...
    $('input[name=advoption]').click(function(){
        hideAdvanced();
        if ($(this).val() == 'particle' && finalPrefix == "Unusual")
        {
...
function classSelect(classname)
{
    ...
    if (step == 1)
    {
        ...
        $(".class-token").each(function (){
            img = $(this).attr('src');
            if ((img === undefined) || (img.lastIndexOf('sm.png') != -1))
                return false;
            image = img.substring(0, img.lastIndexOf('.')) + "sm.png"
            $(this).attr('src', image.toLowerCase());
        });
    }
    step = 2;
